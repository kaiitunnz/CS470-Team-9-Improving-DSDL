...
class Engine(object):
    ...
    def learning(self, model, criterion, train_dataset, val_dataset, test_dataset, optimizer=None):
        ...
        for epoch in range(self.state['start_epoch'], self.state['max_epochs']):
            ...
            if self._state('drive_path') is not None:
                self.save_checkpoint_to_drive({
                    'epoch': epoch + 1,
                    'arch': self._state('arch'),
                    'state_dict': model.module.state_dict() if self.state['use_gpu'] else model.state_dict(),
                    'best_score': self.state['best_score'],
    ...
    def save_checkpoint_to_drive(self, state, is_best, filename='checkpoint.pth.tar'):
        filename_ = filename
        filename = os.path.join(self.state['drive_path'], filename_)
        if not os.path.exists(self.state['drive_path']):
            os.makedirs(self.state['drive_path'])
        print('save model to drive {filename}'.format(filename=filename))
        torch.save(state, filename)
        if is_best:
            filename_best = 'model_best.pth.tar'
            filename_best = os.path.join(self.state['drive_path'], filename_best)
            shutil.copyfile(filename, filename_best)
            if self._state('filename_previous_best') is not None:
                os.remove(self._state('filename_previous_best'))
            filename_best = os.path.join(self.state['drive_path'], 'model_best_{score:.4f}.pth.tar'.format(score=state['best_score']))
            shutil.copyfile(filename, filename_best)
            self.state['filename_previous_best'] = filename_best
